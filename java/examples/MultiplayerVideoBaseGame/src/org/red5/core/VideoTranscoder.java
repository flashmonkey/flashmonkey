...
/**
 * This class takes an IBroadcastStream and modifies each video picture to
 * mirror the right half in the left half, and grayscale the left half.
 */
public class VideoTranscoder {
	...
	final static byte[] mGrayScaleMask = new byte[2048]; // fail if lower that
															// this.
	static {
		// sets the values to set the U and V parts of an image to in order to
		// gray them out.
	...
	/**
	 * Takes each video picture just prior to encoding, and mirrors the right
	 * half over the left half, and grayscales the resulting left half.
	 * <p>
	 * This Video Picture Listener is called every time the {@link Transcoder}
	 * is about to encode a video picture. It allows us to replace or edit the
	 * picture it is encoding. The {@link Transcoder} blocks while this function
	 * executes, so try to be fast.
	 * </p>
	 * <p>
	 * Don't worry if you don't understand all the image math here; the
	 * important thing on first reading is to see that it is possible to modify
	 * images mid-broadcast. Take our word that it works.
	 * </p>
	 */
	final private IVideoPictureListener mVideoPictureListener = new VideoPictureListener() {
		/**
		 * Takes the input picture, and if in YUV420P pixel format, converts it
		 * to half grayscale
		 */
	 ...
	 */
	public VideoTranscoder(String streamPrefix) {
	...
	/**
	 * Starts transcoding this stream. This method is a no-op if this stream is
	 * already a stream copy created by this transcoder.
	 * 
	 * @param aStream
	 *            The stream to copy.
	 * @param aScope
	 ...
	 */
	synchronized public void startTranscodingStream(IBroadcastStream aStream,
			IScope aScope) {
		System.out.println("startTranscodingStream("
				+ aStream.getPublishedName() + "," + aScope.getName() + ")");
		if (aStream.getPublishedName().startsWith(getStreamPrefix())) {
			System.out.println("Not making a copy of a copy: "
					+ aStream.getPublishedName());
			return;
		}
		System.out.println("Making transcoded version of: "
				+ aStream.getPublishedName());
		...
		if (providerService.registerBroadcastStream(aScope, outputName,
			...
			bsScope
					.setAttribute(IBroadcastScope.STREAM_ATTRIBUTE,
							outputStream);
		} else {
			System.out
					.println("Got a fatal error; could not register broadcast stream");
		...
		/**
		 * Now let's give aaffmpeg-red5 some information about what we want to
		 * transcode as.
		 */
		 ...
		 */
		Transcoder transcoder = new Transcoder(aStream, outputStream,
				outputStreamInfo, null, null, mVideoPictureListener);
		...
		System.out.println("Starting transcoding thread for: "
				+ aStream.getPublishedName());
	 ...
	 *            The application scope.
	 */
	synchronized public void stopTranscodingStream(IBroadcastStream aStream,
			IScope aScope) {
		System.out.println("stopTranscodingStream("
				+ aStream.getPublishedName() + "," + aScope.getName() + ")");
